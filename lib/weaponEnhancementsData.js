...
export const weaponEnhancementsData = {
    ...
    cqbOptic: {
        name: "CQB Optic",
        description: "Improves the weapon's accuracy at close and mid range.",
        compatibleWeapons: [
            "all"
        ],
        noticeRollMod: +1,
        apply: (item) => {
            // Add to notes
            const updatedData = {
                "system.notes": addToNotes(item.system?.notes || "", "CQB Optic")
            };
        ...
        remove: (item) => {
            // Remove from notes
            const updatedData = {
                "system.notes": removeFromNotes(item.system?.notes || "", "CQB Optic"),
    ...
    scope: {
        ...
        apply: (item) => {
            // add to notes
            const updatedData = {
                "system.notes": addToNotes(item.system?.notes || "", "Scope")
            };
        ...
        remove: (item) => {
            // remove from notes
            const updatedData = {
                "system.notes": removeFromNotes(item.system?.notes || "", "Scope"),
