...
export const enhancementsData = {
    ...
    slugs: {
        ...
        remove: (item) => {
            ...
            const cached = item.flags?.vjpmacros?.cachedValues || {};
            ...
            updatedData["flags.vjpmacros.cachedValues"] = null;
...
/**
 * Caches the current actions array into item.flags.vjpmacros.cachedData.oldActions.
 * @param {Item} item - The Foundry item to cache the actions for.
 * @returns {Object} - Update data to be passed to item.update().
 */
export function cacheOldActions(item) {
    let currentActions = Array.isArray(item.system.actions) ? item.system.actions : [];
